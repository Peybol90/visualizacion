...
# ─────────────────────────────────────────────────────────────────────────────
# Mapa 4 – Tasa de paro por sección · La Palma · 2021-2024
# Único mapa con 4 años (dataset INE relación actividad)
# ─────────────────────────────────────────────────────────────────────────────

@asset(group_name="maps",
       description="Mapa 4 – Tasa de paro por sección en La Palma · 2021-2024 (facet 4 paneles)")
def mapa_paro_lapalma_2021_2024(relacion_actividad_clean: pd.DataFrame) -> str:
    import pandas as pd
    ...
    # Calcular tasa de paro por sección y año
    ocupados = df[df["relacion"] == "Ocupado/a"].groupby(["año", "geocode"])["num_casos"].sum()
    parados  = df[df["relacion"] == "Parado/a"].groupby(["año", "geocode"])["num_casos"].sum()
    tasa = (parados / (ocupados + parados) * 100).reset_index()
    tasa.columns = ["año", "geocode", "tasa_paro"]
    tasa["tasa_paro"] = tasa["tasa_paro"].round(1)
    tasa["geocode"] = tasa["geocode"].astype(str)

    AÑOS_4 = [2021, 2022, 2023, 2024]
    gdfs = []
    for anyo in AÑOS_4:
        geo_prefix = f"{anyo}0101"
        gdf_lp = _gdf_lapalma_anyo(geo_prefix, anyo)
        t_anyo = tasa[tasa["año"] == anyo][["geocode", "tasa_paro"]].copy()
        merged = gdf_lp.merge(t_anyo, left_on="geocode", right_on="geocode", how="left")
        gdfs.append(merged)
    ...
    p = (
        ggplot(combined)
        + geom_map(aes(fill="tasa_paro"), color="white", size=0.1)
        + scale_fill_gradient(
            low="#FFFFFF", high="#E63946",
            name="Tasa\nparo (%)",
            na_value="#DDDDDD",
        ...
        + labs(
            title="Tasa de paro por sección · La Palma · 2021-2024",
            subtitle="Único indicador con datos hasta 2024  |  INE – Censo Anual de Población tabla 66796",
            caption="Secciones grises: dato no disponible",
        )
        + theme_void()
        + theme(
    ...
    path = f"{OUTPUT_DIR}/mapa_04_paro_lapalma.png"
    p.save(path, dpi=150, width=22, height=8)
    return path
